...
const categories = ["All", "Branding", "Marketing", "Visual Identity", "Product/UIUX Design"];
    ...
    <section className="pt-10 mt-10 container mx-auto px-4 max-w-8xl ">
      ...
      <div className="flex flex-col items-center justify-center text-center mb-20 mx-auto">
        <div className="w-full flex gap-20 justify-center max-w-7xl">
          <p className="max-w-md text-lg text-start font-neuehaaslight">
       ...
       <div className="flex justify-center mb-8">
        {categories.map((category, index) => (
          <button
            key={index}
            className={`flex items-center px-6 py-2 text-sm font-medium border-b-[1px] border-gray-500 ${selectedCategory === category ? "text-gray-500 border-b-2 border-black" : ""}`}
          ...
          {filteredWorks.map((work, index) => (
            <Link key={index} href={`/works/${work.slug || "/"}`}>
              <div className="w-full">
                <div className="relative w-full h-0 pb-[30%] overflow-hidden">
                  <Image
                    src={work.imageSrc}
                    alt={work.title}
                ...
                <div className="flex items-center justify-between py-4">
                  <h3 className="text-lg font-semibold font-neuehaaslight">{work.title}</h3>
                  <span className="border-[1px] text-sm border-black px-4 py-1 whitespace-nowrap rounded-full inline-block font-neuehaaslight">
                    {work.category}
                  </span>
                </div>
                <p className="text-gray-600 font-neuehaasroman">{work.description}</p>
              </div>
            </Link>
          ))}
        </div>
